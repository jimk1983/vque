...
#include "cevtrctor_net_forward.h"


static void accept_cb(int fd, void *pvArgv)
{
    CEvtrctNetForward* net_dispatch = (CEvtrctNetForward*)pvArgv;
...
{
    CEvtrctNetForward* net_dispatch = (CEvtrctNetForward*)pvCtx;
    printf("msqctrl_cb entry!Value=%d\n", Value);
    net_dispatch->dispatch();
    
}

void    CEvtrctNetForward::dispatch()
{
    
    //std::this_thread::sleep_for(std::chrono::seconds(1)); 
}

int     CEvtrctNetForward::network_init(const CEvtRctorCfg& cfg)
{
...
void     CEvtrctNetForward::network_uninit()
{
    if ( -1 != m_listenfd_ )
    {
...
int     CEvtrctNetForward::msque_init(const CEvtRctorCfg& cfg)
{
...
void     CEvtrctNetForward::msque_uninit()
{
    VRCT_API_MsqOptUnRegister(m_rctor_, &m_msqopts_);
}

int      CEvtrctNetForward::messagepost(const int PipeFilterID, const    int Value, const void* Data,const int DataLen)
{
    return VRCT_API_MsqOptPush(m_rctor_, (UINT32_T)PipeFilterID, (UINT32_T)Value, (CHAR *)Data, (UINT32_T)DataLen);
}


int     CEvtrctNetForward::timer_init(const CEvtRctorCfg& cfg)
{
...
void     CEvtrctNetForward::timer_uninit()
{
    VRCT_API_TImerOptUnRegister(m_rctor_, &m_timeropts_);
}
    
int     CEvtrctNetForward::start(const CEvtRctorCfg& cfg)
{
...
CEvtrctNetForward::CEvtrctNetForward():
        m_rctor_(NULL),
        m_taskid_(0),
        m_fliterid_(0),
        m_msqsize_(1000),
