...
#include "cevtrctor_net_dispatch.h"
...
void    CEvtrctNetDispatch::dispatch()
{
    
    //std::this_thread::sleep_for(std::chrono::seconds(1)); 
}

int     CEvtrctNetDispatch::network_init(const CEvtRctorCfg& cfg)
{
...
void     CEvtrctNetDispatch::network_uninit()
{
    if ( -1 != m_listenfd_ )
    {
...
int     CEvtrctNetDispatch::msque_init(const CEvtRctorCfg& cfg)
{
...
void     CEvtrctNetDispatch::msque_uninit()
{
    VRCT_API_MsqOptUnRegister(m_rctor_, &m_msqopts_);
}

int      CEvtrctNetDispatch::messagepost(const int PipeFilterID, const    int Value, const void* Data,const int DataLen)
{
    return VRCT_API_MsqOptPush(m_rctor_, (UINT32_T)PipeFilterID, (UINT32_T)Value, (CHAR *)Data, (UINT32_T)DataLen);
}


int     CEvtrctNetDispatch::timer_init(const CEvtRctorCfg& cfg)
{
...
void     CEvtrctNetDispatch::timer_uninit()
{
    VRCT_API_TImerOptUnRegister(m_rctor_, &m_timeropts_);
}
    
int     CEvtrctNetDispatch::start(const CEvtRctorCfg& cfg)
{
...
CEvtrctNetDispatch::CEvtrctNetDispatch():
        m_rctor_(NULL),
        m_taskid_(0),
        m_fliterid_(0),
        m_msqsize_(1000),
        ...
        m_listenport_(9527)
{
    std::cout << "net dispatch entry!" << std::endl;
}
    
CEvtrctNetDispatch::~CEvtrctNetDispatch()
{
    std::cout << "net dispatch leave!" << std::endl;
}
