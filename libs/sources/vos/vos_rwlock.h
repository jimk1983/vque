...
#define VOS_RWLOCK_TryRdLock(pstRWLock)         (pthread_rwlock_tryrdlock(pstRWLock) ? VOS_SYS_EAGAIN : VOS_OK)
#define VOS_RWLOCK_WrLock(pstRWLock)            pthread_rwlock_wrlock(pstRWLock)
#define VOS_RWLOCK_TryWrLock(pstRWLock)         (pthread_rwlock_trywrlock(pstRWLock) ? VOS_SYS_EAGAIN : VOS_OK)
#define VOS_RWLOCK_TimedRdLock(pstRWLock, sec, ret) do{   \
  struct timespec spec;\
  spec.tv_sec = sec;\
  spec.tv_nsec = 0;\
  ret = pthread_rwlock_timedrdlock(pstRWLock, &spec) ? VOS_SYS_EAGAIN : VOS_OK;  \
}while(0)
...
#define VOS_RWLOCK_TimedWrLock(pstRWLock, sec, ret) do{\
    ...
    ret = pthread_rwlock_timedwrlock(pstRWLock, &spec) ? VOS_SYS_EAGAIN : VOS_OK;\
}while(0)
...
#define VOS_RWLOCK_TryRdLock(pstRWLock)         (TryAcquireSRWLockShared(pstRWLock) ? VOS_OK : VOS_SYS_EAGAIN)
#define VOS_RWLOCK_WrLock(pstRWLock)            AcquireSRWLockExclusive(pstRWLock)
#define VOS_RWLOCK_TryWrLock(pstRWLock)         (TryAcquireSRWLockExclusive(pstRWLock) ? VOS_OK : VOS_SYS_EAGAIN)
#define VOS_RWLOCK_TimedRdLock(pstRWLock, sec, ret) do{\
CONDITION_VARIABLE cv;\
InitializeConditionVariable(&cv);\
ret = SleepConditionVariableSRW(&cv, pstRWLock, sec*1000, CONDITION_VARIABLE_LOCKMODE_SHARED)? VOS_OK : VOS_SYS_EAGAIN;\
}while(0)
#define VOS_RWLOCK_TimedWrLock(pstRWLock, sec, ret) do{\
    CONDITION_VARIABLE cv;\
    InitializeConditionVariable(&cv);\
    ret = SleepConditionVariableSRW(&cv, pstRWLock, sec*1000, 0xffff)? VOS_OK : VOS_SYS_EAGAIN;\
}while(0)
