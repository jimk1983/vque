...
function traffic_monitor {
  ...
  do
    ...
    STATUS="fine"
    # 获取当前时刻网口接收与发送的流量
    RXPackpre=$(cat /proc/net/dev | grep $eth | tr : " " | awk '{print $3}')
    TXPackpre=$(cat /proc/net/dev | grep $eth | tr : " " | awk '{print $11}')
    RXpre=$(cat /proc/net/dev | grep $eth | tr : " " | awk '{print $2}')
    TXpre=$(cat /proc/net/dev | grep $eth | tr : " " | awk '{print $10}')
    # 获取1秒后网口接收与发送的流量
    sleep 1
    RXPacknext=$(cat /proc/net/dev | grep $eth | tr : " " | awk '{print $3}')
    TXPacknext=$(cat /proc/net/dev | grep $eth | tr : " " | awk '{print $11}')
    ...
    RXPack=$((${RXPacknext}-${RXPackpre}))
    TXPack=$((${TXPacknext}-${TXPackpre}))
